  ...
  const handleSubmit = async (e)=>{
    ...
    try {
      ...
      if(data.success === false){
        disptch(updateUserFailure(data.message))
        return
      }
      disptch(updateUserSuccess(data))
      
    } catch (error) {
      disptch(updateUserFailure(error))
    }
  }
    ...
    match /{allPaths=**} {
      ...
      request.resource.contentType.matches('image/.*')
        ...
        onClick={()=>fileRef.current.click()}
         ...
         {
          imageError ? (
            <span className='text-red-700'>Error uploading image(File image must be less than 2 MB)</span>
